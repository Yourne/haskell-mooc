...
substring i j s = let x = take j s in drop i x
...
palindromify s = 
    if reverse s == s
    then s 
    else palindromify (shorten s)

shorten :: String -> String
shorten s = let x = take (length s - 1) s in drop 1 x
...
eitherDiv x y 
    ...
    -- else Right( x/y )
-- eitherDiv x 0 = Left ("Error: division by zero")
-- eitherDiv x y = Right (x / y)
